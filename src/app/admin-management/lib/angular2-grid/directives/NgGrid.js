...
export var NgGrid = (function () {
  ...
  NgGrid.prototype._hasGridCollision = function (pos, dims) {
    return false; // 10 / 03 don't check collision liu jang
  ...
  NgGrid.prototype._limitGrid = function (maxCols) {
    ...
    while (items.length > 0) {
      ...
      var tempColumns = columns.map(function (block) {
        return block.length;
      });
      var currentItems = [];
      while (items.length > 0) {
    ...
    while (this._hasGridCollision(pos, dims) || !this._isWithinBounds(pos, dims)) {
      if (this._hasGridCollision(pos, dims)) {
        ...
        switch (this.cascade) {
          ...
          case 'down':
          default:
            pos.row = Math.max.apply(null, collisions.map(function (item) {
              return item.row + item.sizey;
            }));
            break;
          case 'left':
          case 'right':
            pos.col = Math.max.apply(null, collisions.map(function (item) {
              return item.col + item.sizex;
            }));
            break;
        }
      }
  ...
  NgGrid.prototype._removeFromGrid = function (item) {
    for (var y = item.row; y < item.row + item.sizey; y++)
      for (var x = item.col; x < item.col + item.sizex; x++)
        if (this._itemGrid[y] && this._itemGrid[y][x] == item)
  ...
  NgGrid.prototype._getMaxRow = function () {
    return Math.max.apply(null, this._items.map(function (item) {
  ...
  NgGrid.prototype._getItemFromPosition = function (position, dragStart) {
    for (var _i = 0, _a = this._items; _i < _a.length; _i++) {
      var item = _a[_i];
  ...
  NgGrid.prototype._emitOnItemChange = function () {
    this.onItemChange.emit(this._items.map(function (item) {
      return item.getEventOutput();
    }));
  };
  //	Default config
